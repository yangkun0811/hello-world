...
\d .ctaacd
timedict:`TIME_DELAY`MORNING_TRADE_START`MORNING_TRADE_END`MID_TRADE_START`MID_TRADE_END`AFTNOON_TRADE_START`AFTNOON_TRADE_END`NIGHT_TRADE_START`NIGHT_TRADE_END`FORCE_COVER_START`FORCE_COVER_END!(00:00:30.000;09:00:00.000;10:15:00.000;10:30:00.000;11:30:00.000;13:00:00.000;15:00:00.000;21:00:00.000;01:00:00.000;14:55:00.000;14:59:00.000);
...
// Write log according to strategy id. // cx:建议写入公共策略函数库，如stategylib.q
...
// Round price to the same digit with unit price.// cx:建议写入公共策略函数库，如stategylib.q
round_to_unit_px_ctaacd:{[tid;fsym;price]h:Tx[tid];unitpx:pxunit[fsym];unitpx*`int$(price%unitpx)};

//yk:全天分为早，中，下午,夜盘四个时间段 // cx:建议写入公共策略函数库，如stategylib.q
...
// Check time slot is enable for close.  check_time_status_close_ctaacd[tid] // cx:建议写入公共策略函数库，如stategylib.q
...
// Check time slot is enable for force cover.  check_time_status_forcecover_ctaacd[tid] // cx:建议写入公共策略函数库，如stategylib.q
...
// Check whether the strategy is stopped. check_strategy_status_ctaacd[tid] // cx:建议写入公共策略函数库，如stategylib.q
...
// Block the invalidate price of future. // cx:建议写入公共策略函数库，如stategylib.q
...
//check position;check_position_ctaacd[tid]  // cx:建议写入公共策略函数库，如stategylib.q
...
// Update quote dictionary when new quote data arrives.  // cx:生成k线，建议写入公共策略函数库，如stategylib.q
update_quote_dict_ctaacd:{[tid] 
    h:Tx[tid];
    freq:h`FREQ; // cx:k线合成单位应该是秒
    futcode:h`FUT;
...
update_forcecover_condition_ctaacd:{[tid]
	h:Tx[tid];
	if[not h`EnableForceCover;:()]; // cx:EnableForceCover 这个标志什么用？
...
onq_ctaacd:{[tid;y]
    ...
    if[(Tx[tid;`LASTTICKTIME]<>QX[h[`SYMS];`time])&(check_time_status_open_ctaacd[tid]); // cx:check_time_status_open_ctaacd 这个函数导致的无行情情况，请打印错误日志。
    	Tx[tid;`LASTTICKTIME]:QX[h[`SYMS];`time];
		update_quote_dict_ctaacd[tid]; 
		if[h[`WATCHMOD]=0b;  // cx:如何设计监控界面？可以考虑监控CTA所有策略的策略运行情况、仓位表情况，无需图形化界面。
			if[h[`ENABLEORDER]=1b;
				update_long_short_condition_ctaacd[tid];
				//update_close_condition_ctaacd[tid];
				update_forcecover_condition_ctaacd[tid];
...
ono_ctaacd:{[tid;orderid]
    ...
    if[status=.enum`FILLED;update_order_table_in_filled_status_ctaacd[tid;orderid]]; // cx:下单完成后，futid在哪里删除？
    if[status=.enum`CANCELED;update_order_table_in_cancelled_status_ctaacd[tid;orderid]]; // cx:新下单的单号id如何处理？
	};
...
ond_ctaacd:{[tid;date]
	...
	Tx[tid;`A]:(Tx[tid;`DHigh]-Tx[tid;`DLow])%(.ctaacd.paramdict[`stoplossfactor]); // cx:如何判定除0错误？为何在ond写A参数？
...
//active_strategy_ctaacd[tid] tid:`cta // cx:此函数为手动active策略，写操作日志。
active_strategy_ctaacd:{[tid]
	T[tid;`active]:1b;
	T[tid;`stop]:`;
	Tx[tid;`CTAWORKSTATUS]:1b;
	Tx[tid;`CTARESSTATUS]:`;  // cx:字符串型？
	Tx[tid;`ENABLEORDER]:1b;
	Tx[tid;`LSFLAG]:0i;
    // cx:是否有其他变量强制初始化，如forcecover?
	};
...
// cx:参数列需要统一大小写，以后参数变量全部大写。
// cx:极端行情中如有平仓平不掉情况，如期货涨跌停如何处理？